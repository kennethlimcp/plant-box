...
boolean SPI_Motor::sendToCard(uint8_t CMD, uint8_t DATA) {
    ...
    pinSetFast(_CS);//set CS High so the data gets latched in the card
    pinResetFast(_CS); //set CS low, now we clear out the data so it doesn't effect other cards.
